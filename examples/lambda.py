...
comparison_name = "jasmin_daily-pacific-four_outputs_liqcf"
...
for k, v in experiment_dict.items():
    ...
    log_lambda = 0.5
    apo_limits_3_path = ensemble_dir / "apo_limits_0.5.npy"
    ...
    # log_lambda = 1.0
    # apo_limits_4_path = ensemble_dir / "apo_limits_1.0.npy"
    ...
    # apo_limits_4_dict[k] = apo_limits_4
...
for idx_outcome in range(len(target_keys_dict[comparison_key])):
    ...
    for k in experiment_dict.keys():
        ...
        if k == comparison_key:
            ...
            _ = ax[i][j].fill_between(
                ...
                y1=scaler.transform(
                    np.quantile(
                        apo_limits_infty_dict[k][1][idx_outcome], 1 - alpha / 2, axis=0
                    ).reshape(-1, 1)
                ).flatten(),
                y2=scaler.transform(
                    np.quantile(
            ...
            _ = ax[i][j].fill_between(
                ...
                y1=scaler.transform(
                    np.quantile(
                        apo_limits_infty_dict[k][0][idx_outcome], 1 - alpha / 2, axis=0
                    ).reshape(-1, 1)
                ).flatten(),
                y2=scaler.transform(
                    np.quantile(
    ...
    _ = ax[i][j].set_xlabel(ds["train"].treatment_names[0])
    _ = ax[i][j].set_ylabel(target_keys_dict[comparison_key][idx_outcome])
plt.savefig(f"{comparison_name}-infty.pdf")
plt.show()
...
for idx_outcome in range(len(target_keys_dict[comparison_key])):
    ...
    _ = ax[i][j].set_xlabel(ds["train"].treatment_names[0])
    _ = ax[i][j].set_ylabel(target_keys_dict[comparison_key][idx_outcome])
plt.savefig(f"{comparison_name}-1.6.pdf")
plt.show()