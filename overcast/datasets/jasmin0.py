...
class JASMINLowResolution(data.Dataset):
    def __init__(
        self,
        root: str,
        split: str,
...
class JASMINDailyLowResolution(data.Dataset):
    def __init__(
        self,
        root: str,
        split: str,
        ...
        super(JASMINDailyLowResolution, self).__init__()
        # Handle default values
        if x_vars is None:
            x_vars = [
                "RH900",
...
class JASMINHighResolution(data.Dataset):
    def __init__(
        self,
        root: str,
        split: str,
        ...
        days_train = set(days).difference(days_valid.union(days_test))
        # Fit preprocessing transforms
        df_train = df[df["dates"].isin(days_train)]
        self.data_xfm = preprocessing.StandardScaler()
        self.data_xfm.fit(df_train[x_vars].to_numpy())
        self.treatments_xfm = (
            preprocessing.KBinsDiscretizer(n_bins=t_bins, encode="onehot-dense")
        ...
        # Split the data
        if split == "train":
            _df = df[df["dates"].isin(days_train)]
        elif split == "valid":
            _df = df[df["dates"].isin(days_valid)]
        elif split == "test":
            _df = df[df["dates"].isin(days_test)]
        # Set variables
        self.data = self.data_xfm.transform(_df[x_vars].to_numpy(dtype="float32"))
        self.treatments = self.treatments_xfm.transform(
            _df[t_var].to_numpy(dtype="float32").reshape(-1, 1)
...
class JASMINDailyHighResolution(data.Dataset):
    def __init__(
        self,
        root: str,
        split: str,
        ...
        super(JASMINDailyHighResolution, self).__init__()
        # Handle default values
        if x_vars is None:
            x_vars = [
                "MERRA_RH950",
        ...
        for _, group in _df:
            if len(group) > 1:
                targets = self.targets_xfm.transform(
                    group[y_vars].to_numpy(dtype="float32")
                )
                if np.isnan(targets).sum() != (lambda x: x[0] * x[1])(
                    targets.shape
                ):  # target is not full of nan
                    self.data.append(
                        self.data_xfm.transform(group[x_vars].to_numpy(dtype="float32"))
                    )
                    self.treatments.append(
                    ...
                    self.position.append(
                        group[["lats", "lons"]].to_numpy(dtype="float32")
                    )
        # Variable properties
        self.dim_input = self.data[0].shape[-1]
        self.dim_targets = self.targets[0].shape[-1]
