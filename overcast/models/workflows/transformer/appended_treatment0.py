...
def train(
    ...
    experiment_dir = (
        Path(experiment_dir)
        / f"dh-{dim_hidden}_nco-{num_components_outcome}_nct-{num_components_treatment}_dp-{depth}_nh-{num_heads}_ns-{negative_slope}_bt-{beta}_ln-{layer_norm}_dr-{dropout_rate}_sn-{spectral_norm}_lr-{learning_rate}_bs-{batch_size}_ep-{epochs}"
    )
    ...
    if not (out_dir / "best_checkpoint.pt").exists():
        outcome_model = models.AppendedTreatmentAttentionNetwork(
            job_dir=out_dir,
